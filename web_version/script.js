...
function aiTurn() {
  ...
  if (emptyCells(board).length == 9) {
    ...
    var cc = document.getElementById("depth").value;
    if (cc == 100) {
      Gamedepth = emptyCells(board).length;
    } else {
      Gamedepth = cc;
    }
    //console.log(emptyCells(board).length);
    move = minimax(board, Gamedepth, COMP);
...
function clickedCell(cell) {
  ...
  if (gameOver(board, COMP)) {
    ...
    document.getElementById("result").src = "images/loss.gif";
  ...
  if (gameOverAll(board) == true || emptyCells(board).length == 0) {
    ...
    if (emptyCells(board).length !== 0) {
      document.getElementById("result").src = "images/win.gif";
      var msg = document.getElementById("message");
...
function restartBnt(button) {
  if (button.value == "Start AI") {
    ...
    msg = document.getElementById("message");
    msg.innerHTML = "";
    document.getElementById("result").src = "images/bugs.png";
  }
}
